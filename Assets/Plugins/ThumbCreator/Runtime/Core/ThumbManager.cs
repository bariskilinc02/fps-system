...
namespace ThumbCreator.Core
{
    ...
    public class ThumbManager : MonoBehaviour
    {
        ...
        {
            if (ExportFile == FileType.Sprite)
            {
                transform.localRotation = Quaternion.Euler(RotationX, 0, RotationZ);
                Screenshot.GeneratePng(Camera.main,Filename, m_width, m_height, false, 0);
                return;
            }
            if (ExportFile != FileType.Png)
            {
                ...
                for (int i = 0; i < 360; i += frameCount)
                {
                    ...
                    Screenshot.GeneratePng(Camera.main,Filename, m_width, m_height, false, count);
                    count++;
                }
            }
        }
        ...
        public void GenerateFile()
        {
            switch (ExportFile)
            {
                ...
                case FileType.Sprite:
                    ...
                    //tempTexture = Screenshot.GeneratePng(Filename, m_width, m_height);
        ...
        private void GenerateSpriteNew(Texture2D tex)
        {
            ...
            ConvertedSprite.sprite = Sprite.Create(tex,new Rect(0, 0, m_width, m_height),new Vector2(50,50), 200f);
